...
import { FormField, LocalePair, parseNumber, readI18n, type CatalogEditorProps } from './shared'

export function DrinksProductEditor({ item, onDelete, onSaved, embedded = false }: CatalogEditorProps) {
  const [product, setProduct] = useState(item)
  const meta = product.metadata ?? {}
  const [unit, setUnit] = useState(String(meta.unit ?? 'L'))
  const [minQty, setMinQty] = useState(String(meta.minQty ?? 1))
    ...
    <Card>
      <CardHeader>
        <CardTitle className="text-base">
          {nameEn || product.name}{' '}
          <span className="text-xs text-muted-foreground">(Drinks Product)</span>
        </CardTitle>
      </CardHeader>
