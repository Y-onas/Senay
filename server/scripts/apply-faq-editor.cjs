...
const G6 = fs
  .readFileSync(path.join(__dirname, 'faq-editor-g6.min.js'), 'utf8')
  .replace(/^"use strict";/, '')
  .replace(/^function \w+\(/, 'function g6(')
...
for (const rel of bundles) {
  ...
  const start = text.indexOf('function g6()')
  const end = text.indexOf('function y6()')
  if (end === -1) {
    console.error('y6 anchor missing in', rel)
    process.exit(1)
  }
  const next =
    start === -1
      ? text.slice(0, end) + G6 + text.slice(end)
      : text.slice(0, start) + G6 + text.slice(end)
