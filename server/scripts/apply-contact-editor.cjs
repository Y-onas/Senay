...
const H6 = fs
  .readFileSync(path.join(__dirname, 'contact-editor-h6.min.js'), 'utf8')
  .replace(/^"use strict";/, '')
  .replace(/^function \w+\(/, 'function h6(')
...
for (const rel of bundles) {
  ...
  const start = text.indexOf('function h6()')
  const end = text.indexOf('function a5()')
  if (end === -1) {
    console.error('a5 anchor missing in', rel)
    process.exit(1)
  }
  const next =
    start === -1
      ? text.slice(0, end) + H6 + text.slice(end)
      : text.slice(0, start) + H6 + text.slice(end)
