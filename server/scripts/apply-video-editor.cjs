...
const V6 = fs
  .readFileSync(path.join(__dirname, 'video-editor-v6.min.js'), 'utf8')
  .replace(/^"use strict";/, '')
  .replace(/^function \w+\(/, 'function v6(')
...
for (const rel of bundles) {
  ...
  const start = text.indexOf('function v6(')
  const end = text.indexOf('function s6(')
  if (end === -1) {
    console.error('s6 anchor missing in', rel)
    process.exit(1)
  }
  const next =
    start === -1
      ? text.slice(0, end) + V6 + text.slice(end)
      : text.slice(0, start) + V6 + text.slice(end)
