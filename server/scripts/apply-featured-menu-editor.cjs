...
const F6 = fs
  .readFileSync(path.join(__dirname, 'featured-menu-editor-f6.min.js'), 'utf8')
  .replace(/^"use strict";/, '')
  .replace(/^function \w+\(/, 'function f6(')
...
for (const rel of bundles) {
  ...
  const start = text.indexOf('function f6(')
  const end = text.indexOf('function o6(')
  if (end === -1) {
    console.error('o6 anchor missing in', rel)
    process.exit(1)
  }
  const next =
    start === -1
      ? text.slice(0, end) + F6 + text.slice(end)
      : text.slice(0, start) + F6 + text.slice(end)
